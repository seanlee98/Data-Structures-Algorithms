...
D = new double[size];
pqueue = new Priority_Queue(number_of_vertices);
...
double Weighted_graph::adjacent(int m, int n) const {
	if(m < 0 || m >= number_of_vertices || n < 0 || n >= number_of_vertices)
...
double Weighted_graph::distance(int m, int n) {
	...
	while(!pqueue->empty()) {
		u = pqueue->getMin();//Gets the minimum value from the heap and also removes it		
		pqueue->deleteMin();						 //u is a pair of values (vertex, weight)
		//std::cout << "current node is: " << u.vertex() << std::endl;
		//std::cout << "current distance is: " << u.weight() << std::endl;
			...
			for(int j = 0; j < number_of_vertices; ++j){
				...
				if(adjValue != INF){
						double newDist =  D[u.vertex()] + adjValue;
						if(newDist < D[j]){
							D[j] = newDist;		
							pqueue->insert(j, newDist);	
							//std:: cout << "node being updated is: " << j << std::endl;
							//std:: cout << "new distance is: " << D[j] << std::endl;
						}
					}
